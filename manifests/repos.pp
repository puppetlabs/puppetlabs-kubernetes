...
class kubernetes::repos (
  ...
  Optional[String] $crio_ppa_repository_name = $kubernetes::crio_ppa_repository_name,
  Optional[String] $crio_yum_baseurl         = $kubernetes::crio_yum_baseurl,
  Boolean $manage_docker                     = $kubernetes::manage_docker,
  Boolean $manage_crio                       = $kubernetes::manage_crio,
  ...
  if $create_repos {
    case $facts['os']['family'] {
      'Debian': {
        ...
        if $container_runtime == 'crio' and $manage_crio == true {
          apt::ppa { $crio_ppa_repository_name:
          }
        }
      }
      'RedHat': {
