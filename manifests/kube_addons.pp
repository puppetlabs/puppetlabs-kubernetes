...
class kubernetes::kube_addons (
  ...
  $node_ready_command = "kubectl get nodes|egrep -v  'NAME|Ready'|egrep '.*'"
  ...
  if $cni_network_provider {
    ...
    exec { 'Install cni network provider':
      ...
      unless      => [$node_ready_command, "kubectl -n kube-system get daemonset | egrep '(flannel|weave|calico-node|cilium)'"],
      environment => $env,
    }
  }
