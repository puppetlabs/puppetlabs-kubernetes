...
def read_admissionregistration_v1beta1_validating_webhook_configuration(*args)
  header_params = {}
  ...
  uri_string = "#{arg_hash['kube_api']}/apis/admissionregistration.k8s.io/v1beta1/validatingwebhookconfigurations/%{name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
    op_param('admissionreviewversions', 'body', 'admissionreviewversions', 'admissionreviewversions'),
    op_param('clientconfig', 'body', 'clientconfig', 'clientconfig'),
    op_param('exact', 'query', 'exact', 'exact'),
    op_param('export', 'query', 'export', 'export'),
    op_param('failurepolicy', 'body', 'failurepolicy', 'failurepolicy'),
    op_param('name', 'path', 'name', 'name'),
    op_param('namespaceselector', 'body', 'namespaceselector', 'namespaceselector'),
    op_param('pretty', 'query', 'pretty', 'pretty'),
    op_param('rules', 'body', 'rules', 'rules'),
    op_param('sideeffects', 'body', 'sideeffects', 'sideeffects'),
    op_param('timeoutseconds', 'body', 'timeoutseconds', 'timeoutseconds'),
  ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = $stdin.read
  result = read_admissionregistration_v1beta1_validating_webhook_configuration(params)
