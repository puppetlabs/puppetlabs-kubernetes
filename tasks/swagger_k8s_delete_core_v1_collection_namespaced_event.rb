...
def delete_core_v1_collection_namespaced_event(*args)
  header_params = {}
  ...
  uri_string = "#{arg_hash['kube_api']}/api/v1/namespaces/%{namespace}/events" % path_params
...
def format_params(key_values)
  ...
  op_params = [
    ...
    op_param('firsttimestamp', 'body', 'firsttimestamp', 'firsttimestamp'),
    op_param('involvedobject', 'body', 'involvedobject', 'involvedobject'),
    op_param('kind', 'body', 'kind', 'kind'),
    op_param('labelSelector', 'query', 'label_selector', 'label_selector'),
    op_param('lasttimestamp', 'body', 'lasttimestamp', 'lasttimestamp'),
    ...
    op_param('reportingcomponent', 'body', 'reportingcomponent', 'reportingcomponent'),
...
def task
  ...
  params = $stdin.read
  result = delete_core_v1_collection_namespaced_event(params)
