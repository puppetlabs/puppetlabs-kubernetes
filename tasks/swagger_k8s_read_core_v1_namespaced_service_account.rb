...
def read_core_v1_namespaced_service_account(*args)
  header_params = {}
  ...
  uri_string = "#{arg_hash['kube_api']}/api/v1/namespaces/%{namespace}/serviceaccounts/%{name}" % path_params
...
def format_params(key_values)
  ...
  op_params = [
    ...
    op_param('fieldpath', 'body', 'field_path', 'fieldpath'),
    ...
    op_param('resourceversion', 'body', 'resource_version', 'resourceversion'),
    op_param('uid', 'body', 'uid', 'uid'),
  ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = $stdin.read
  result = read_core_v1_namespaced_service_account(params)
